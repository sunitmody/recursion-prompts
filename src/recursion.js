...
var fibonacci = function(n) {
  ...
  let prevArray = fibonacci(n-1);
  let lastNum = prevArray[prevArray.length-1]
  let oneBeforeLastNum = prevArray[prevArray.length-2]

  return prevArray.concat([lastNum+oneBeforeLastNum]);
};
...
var capitalizeFirst = function(array) {
  ...
  //first letter of last word in array capitalized
  let word = array[array.length-1];
  let capitalizedWord = word[0].toUpperCase() + word.slice(1);

  return capitalizeFirst(smallerArr).concat([capitalizedWord]);
};

/*
########################################################
CAPITALIZEFIRST COMPLETED
########################################################
*/
...
var nestedEvenSum = function(obj) {
  ...
  for (k in obj) {
    ...
    //if current key's value is even
    if (obj[k]%2 === 0) {
      sum += obj[k];
    }
